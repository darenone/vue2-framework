const saveLocal = store => {
  ...
  console.log('store初始化', localStorage.activedTabs)
  if (localStorage.activedTabs) {
    // store.replaceState(JSON.parse(sessionStorage.state)) // 全部保存
    store.state.activedTabs = localStorage.activedTabs // 只保存某个值
  }
  ...
  store.subscribe((mutation, state) => {
    ...
    localStorage.activedTabs = JSON.stringify(state.activedTabs) // 只保存某个值
  })
}
