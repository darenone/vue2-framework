...
注意：此vue2框架搭建于node版本[18.12.1]npm版本[8.19.2]，在此环境下不会报错，如果在其它node版本下出现问题，大概率是引用的开发依赖版本不兼容导致，需要针对具体报错信息进行处理，遇到问题去搜索针对性解决即可
...
在多人开发中，为了保证代码格式统一和保证代码质量，需要在项目里配置`.eslintrc.js`和`.eslintignore`，详细见[.eslintrc.js](.eslintrc.js)，但是这种方式不能格式化html和css文件，`prettier`可以用来格式化html和css

在项目里新建`.prettierrc.js`和`.prettierignore`

eslint和prettier一起使用，可能会存在冲突，为了解决这个冲突，需要在`.eslintrc.js`里配置以下规则：
...
如果不是用vscode编辑器自动保存时格式化代码，就需要使用命令行对文件进行格式化：
...
```
**关于`.eslint.js`和`.prettierrc`文件**

如果你使用的前端开发工具是vscode，且安装了vscode的拓展插件：`Prettier - Code formatter`和`ESLint`，那么在`CTR+S`保存的时候，会依据`.eslint.js`和`.prettierrc`里的配置，自动进行格式化，如果项目里没有配置这两个文件，会依靠vscode的`setting.json`里配置，结合vscode的拓展插件`Vetur`和`ESLint`，对代码进行格式化，vscode的详细配置见[settings.json](附录03：vscode的setting配置.md)
...
```js
// 此依赖只作为开发环境使用，所以后缀不是--save 而是-D
cnpm/npm install mockjs -D
```

## 四、配置路由

项目里详细的路由配置见文件[附录05：vue项目配置路由的几种方式.md](附录05：vue项目配置路由的几种方式.md)
...
有一个创建任务的页面，路由为：http://localhost:8082/#/web-task/task-create
分别可以从两个页面：监测任务列表：http://localhost:8082/#/web-task/web-list
和监测任务管理：http://localhost:8082/#/web-task/task-list
跳转过来，需求是，从哪个页面跳转过来的，当任务创建完毕还回到哪个页面
...
```
但是我发现在里面拿不到this这个vue实例，是因为：走这一步的时候，当前组件还没有渲染完成，vue实例还未创建完成，解决方法就是给next函数传一个回调函数，完美解决这个问题
```js
beforeRouteEnter (to, from, next) {
  next(vm => {
...
新建`src\store\module\i18n.js`异步加载放在服务端获取的国际化文件（由于本项目没有接入后端接口，国际化文件放在`src\i18n\lang`），在`App.vue`里触发方法`loadLanData`，之后在`main.js`里引入
...
新建`src\assets\js\elementui\index.js`，并在`main.js`里面引入，如果有重写elementui全局样式的需求，可以放到[src\assets\css\element-ui.scss](src\assets\css\element-ui.scss)里面，再把这个文件放到[src\assets\css\index.scss](src\assets\css\index.scss)，再在`main.js`里引入`index.scss`即可，以上elementui就引入成功

## 十、手写一个导航菜单

这里我不利用任何外部UI组件来实现一个导航菜单，帮助学习vue的开发，只要可以手写一个菜单，其它的菜单照着这个思路，也是没有问题的

访问这个路由：http://localhost:8080/#/menu
就可以看到我自己封装的menu组件长啥样

新建[menu/index.vue](src\views\module\menu\index.vue)，在这个页面引入：[components/menu/index.vue](src\components\menu\index.vue)和[components/menu/menu-item.vue](src\components\menu\menu-item.vue)和[components/menu/re-submenu.vue](src\components\menu\re-submenu.vue)，导航菜单代码就在这两个文件里，具体的逻辑看这几个文件即可
...
[附录05：vue项目配置路由的几种方式.md](附录05：vue项目配置路由的几种方式.md)
...
`mock.js`在项目搭建的时候就已经安装，上面的导航菜单就用到了mock数据

具体配置见：[src\mock\index.js](src\mock\index.js)

然后在`main.js`里引入即可，在组件里发起请求：
例如项目里[src\store\module\info.js](src\store\module\info.js)的`SystemApi.info()`和`SystemApi.menu()`这两个接口就是走的mock逻辑，具体用法可参考这两个写法，记得首先需要配置对应的api：[src\api\SystemApi.js](src\api\SystemApi.js)
这个文件下也有示例：
