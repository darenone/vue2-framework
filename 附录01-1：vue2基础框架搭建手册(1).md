...
如果你使用的前端开发工具是vscode，且安装了vscode的拓展插件`Prettier - Code formatter`和`ESLint`，那么在ctr+s保存的时候，会依据`.eslint.js`和`.prettierrc`里的配置，自动进行格式化，如果项目里没有配置这两个文件，会依靠vscode的setting.json里配置，结合vscode的拓展插件`Vetur`和`ESLint`，对代码进行格式化

如果需要要想让vscode编辑器，在保存代码时自动格式化代码，项目里需要安装`babel-eslint`，配置项目里的`.eslintrc.js`文件，vscode还需要安装`vetur`和`eslint`拓展插件，还需要设置vscode的[settings.json](附录03：vscode的setting配置.md)
